...
c0 = mean(primitive.c)
t_i = (x_probe_i - x0) / c0
inferred_wave_speed_i = (x_probe_i - x0) / numerical_arrival_time_i
```
...
初回 verification では **baseline からの圧力変化が probe peak amplitude の 50% を初めて超える時刻**を `numerical_arrival_time_s` とする。
...
この方法は実装が簡単で、peak 時刻より数値拡散に強い。注意点として、threshold は pulse 幅・拡散・小反射に依存するため、初回では 30%, 50%, 70% crossing の感度を debug metadata に残せると望ましい。ただし正式 metric は 50% crossing に固定する。
