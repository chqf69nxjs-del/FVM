...
## 12. Formal verification report generator

正式sweep実行後の `*_sweep_report.md` は run 直後の速報summaryとして維持する。正式なtechnical interpretation documentは、既存の sweep metrics JSON と summary CSV を入力にして `generate_coolprop_small_amplitude_wave_verification_report(...)` で再生成する。
...
- `coolprop_small_amplitude_wave_verification_report_v1.md`: formal verification report。
