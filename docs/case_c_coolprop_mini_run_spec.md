...
e0 = PropsSI("Umass", "P", p0, "T", T0, "CO2")
```

CoolProp 未導入環境では、`rho0` と `e0` の実値を推測して記載しない。mini-run test または実行 entry point は、CoolProp 未導入時に明示的に skip する。
...
| `internal_energy_j_kg` | CoolProp `PropsSI("Umass", ...)` から生成 | 固定値を仕様書に書かない。 |
...
1. mini-run 専用 builder で `CoolPropCO2Backend().density_from_pT(p0, T0)` と `PropsSI("Umass", ...)` から `U0` を作り、既存 `FvmSolver` に渡す。
