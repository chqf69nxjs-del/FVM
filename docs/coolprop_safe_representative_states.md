...
本書は、`coolprop_co2` backend を用いた Case C 最小試走に向けて、CoolProp API の software-path verification で扱いやすい CO₂ 代表状態候補を整理する開発メモである。主な目的は、`density_from_pT()`、`state_from_rho_e()`、`saturation_state()` の呼び出し経路を、数値的に過度に難しい状態を避けて確認することである。
...
この状態は、既存の installed-only test で使用されている単相圧縮液体 CO₂ 条件である。テスト上も「飽和線および臨界点から離して、CoolProp API smoke check を数値的に保守的にする」意図で選ばれている。

確認事項は以下である。

- CoolProp 導入環境で単相液体として評価されること。
- 臨界点・飽和線近傍の曖昧な状態を避けられること。
...
|---|---:|---:|---|---|---|---|---|---|---|---|
| A. 単相圧縮液体 | 8.0 MPa | 280 K | CoolProp 導入環境で取得予定 | CoolProp 導入環境で取得予定 | CoolProp 導入環境で取得予定 | CoolProp 導入環境で取得予定 | 単相液体として確認予定。CoolProp `Q` が単相特別値を返す場合は backend 側の quality 解釈を確認する | `T - T_sat(8.0 MPa)` を CoolProp 導入環境で確認予定 | Case C mini-run 第一候補、`density_from_pT()` / `state_from_rho_e()` 往復確認 | `not_approved_for_design_use` |
...
保存時は、少なくとも `p`、`T`、`rho`、`e`、`h`、sound speed、phase / quality、`T - T_sat(p)`、使用目的、design-use status を同一レコードに残す。特に、`coolprop_co2` の結果であっても design-use status を必ず併記する。
...
現時点の Case C mini-run 第一候補は、既存 installed-only test と同じ 8.0 MPa、280 K の単相圧縮液体状態とする。
...
| 初期内部エネルギー | CoolProp `PropsSI("Umass", "P", 8.0e6, "T", 280.0, "CO2")` で取得する。今回環境では CoolProp 未導入のため実値は CoolProp 導入環境で取得予定 |
...
| 選定理由 | 既存 installed-only test で採用済みの保守的な単相圧縮液体条件であり、飽和線・臨界点近傍の曖昧さを避けつつ、`density_from_pT()` と `state_from_rho_e()` の往復確認に使えるため |
| 注意点 | 本書の条件は software-path verification 用であり design-use 条件ではない。CoolProp 導入環境で `rho`、`e`、sound speed、phase / quality、`T - T_sat(p)` を取得・保存してから mini-run に使う |

コード構造上、`CoolPropCO2Backend.density_from_pT()` は `PropsSI("Dmass", "P", p, "T", T, "CO2")` を呼び、`state_from_rho_e()` は `PropsSI("P"/"T"/"Q"/"A", "Dmass", rho, "Umass", e, "CO2")` を呼ぶ。したがって 8.0 MPa、280 K は、`p, T` から `rho` を作り、同じ状態の `e` と組み合わせて `rho, e` flash の戻り値を確認する software-path verification に適している。
