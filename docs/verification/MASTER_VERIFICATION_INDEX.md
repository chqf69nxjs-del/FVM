...
- V-012 implementation-ready specificationはPR #34で進行中。既存Kv則、opening schedule、two-sided interface flux、zero-opening wall degeneration、Mach cap、diagnostic energy limitationを再確認済み。
...
1. PR #34でimplementation-ready specificationをレビューしてマージする。
2. diagnostic-only raw/applied/capped flow helperとtwo-sided interface-flux telemetryを追加する。
3. constant-opening CoolProp baseline runnerを小規模PRで実装する。
...
| V-012 | Single-phase valve operation | IN_PROGRESS | Stage 6 base spec、PR #34 refined spec、existing Kv/interface survey | operation runner、telemetry、observations未実装 | constant-opening baseline runner |
...
- left/right initial pressures `8,000,500 / 7,999,500 Pa` at `280 K`
- full-open Kv derived for `1.0e-3 m/s` target face velocity at the initial `1 kPa` pressure difference
...
| Stage 6 | IN_PROGRESS | V-012 constant/ramp runner、mesh/CFL、CI、formalization |
