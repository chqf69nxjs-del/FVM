"""Configuration helpers for V-012C controlled internal-valve opening."""
...
class CoolPropInternalValveOpeningRampConfig:
    """Numerical-observation inputs for the primary 0 -> 1 opening ramp."""

    case_name: str = "coolprop_internal_valve_opening_ramp"
    output_version: str = "coolprop_internal_valve_opening_ramp_v1"
    ...
    open_initial: float = 0.0
    open_final: float = 1.0
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 <= self.open_initial < self.open_final <= 1.0:
            raise ValueError("opening ramp must satisfy 0 <= initial < final <= 1")
