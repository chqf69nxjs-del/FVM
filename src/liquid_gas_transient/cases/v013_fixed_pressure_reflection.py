"""Pure V-013C fixed-pressure reflection specification and run-plan helpers.

This module fixes the Stage 7 / V-013C observation contract before the production
...
ReflectionPhase = Literal["incident", "boundary_contact", "reflected"]


@dataclass(frozen=True)
class V013FixedPressureReflectionConfig:
    """Fixed observation configuration for Stage 7 / V-013C."""

    case_name: str = "v013c_fixed_pressure_reflection"
    output_version: str = "v013c_fixed_pressure_reflection_v1"
    matched_sample_schema_version: str = "v013c_matched_samples_v1"
    ...
    def __post_init__(self) -> None:
        ...
        if self.pressure_amplitude_pa / self.initial_pressure_pa > 1.0e-4:
            raise ValueError(
                "V-013C pressure perturbation must remain in the linear guardrail"
            )
        ...
        if not any(
            math.isclose(distance, self.boundary_path_travel_m, abs_tol=1.0e-10)
            for distance in self.matched_path_travel_m
        ):
            raise ValueError(
                "matched_path_travel_m must include the fixed-pressure contact distance"
            )
        if self.matched_path_travel_m[-1] <= self.boundary_path_travel_m:
            raise ValueError(
        ...
        if self.validation or self.design_evaluation or self.acceptance_gate:
            raise ValueError("V-013C validation/design/acceptance flags must remain false")

        guard_m = self.boundary_guard_sigma * self.pulse_sigma_m
        if self.pipe_length_m - self.pulse_center_m <= guard_m:
            raise ValueError("the initial Gaussian is too close to the pressure boundary")

        closest_probe_to_boundary_m = self.pipe_length_m * max(self.probe_fractions)
        minimum_event_separation_m = self.pipe_length_m - closest_probe_to_boundary_m
        ...
        for distance in self.matched_path_travel_m:
            ...
            if (
                ...
                raise ValueError(
                    "an incident matched sample enters the pressure-boundary guard envelope"
                )
            if phase == "reflected":
                if center_m + guard_m > self.pipe_length_m + 1.0e-12:
                    raise ValueError(
                        "a reflected matched sample remains inside the boundary-contact envelope"
                    )
    ...
    @property
    def boundary_path_travel_m(self) -> float:
...
def case_id_for(
    ...
    """Return the stable V-013C case identifier for one mesh."""
    ...
    return (
        f"v013c_n{int(n_cells):04d}_"
        f"fvmcfl{_float_token(fvm)}_moccfl{_float_token(moc)}"
    )


def fixed_pressure_expected_conditions() -> dict[str, Any]:
    ...
    return {
        ...
        "boundary_pressure_perturbation_pa": 0.0,
        "total_boundary_velocity_to_incident_velocity_ratio": 2.0,
...
def _path_state_unchecked(
    path_travel_m: float,
    config: V013FixedPressureReflectionConfig,
) -> dict[str, Any]:
    distance = float(path_travel_m)
    boundary_distance = config.boundary_path_travel_m
    if distance < boundary_distance and not math.isclose(
        distance, boundary_distance, abs_tol=1.0e-10
    ):
        phase: ReflectionPhase = "incident"
        center_m = config.pulse_center_m + distance
        dominant_characteristic = "A+"
    elif math.isclose(distance, boundary_distance, abs_tol=1.0e-10):
        phase = "boundary_contact"
        center_m = config.pipe_length_m
        dominant_characteristic = "A+ and A- with opposite sign"
    else:
        phase = "reflected"
...
def reflection_path_state(
    ...
    return {
        **state,
        ...
        "primary_boundary_guard_overlap_expected": bool(
            phase == "boundary_contact"
...
def build_run_plan(
    ...
    """Build the fixed three-run V-013C mesh plan."""
...
def build_matched_sample_plan(
    ...
    for index, distance in enumerate(cfg.matched_path_travel_m):
        ...
        rows.append(
            {
                "sample_id": f"v013c_sample_{index:02d}_{_float_token(distance)}m",
                "verification_item": "V-013C",
                "matched_sample_schema_version": cfg.matched_sample_schema_version,
                **state,
...
def build_probe_plan(
    ...
    for fraction in cfg.probe_fractions:
        ...
        if not incident_end_s < boundary_start_s:
            raise RuntimeError("incident and boundary-contact windows are not separated")
        if not boundary_end_s < reflected_start_s:
            raise RuntimeError("boundary-contact and reflected windows are not separated")
        rows.append(
            {
                ...
                "right_reflection_second_return_time_s": float(
                    second_boundary_return_path_m / c0
                ),
...
def build_specification_snapshot(
    ...
    return {
        ...
        "status": "IN_PROGRESS",
        "config": asdict(cfg),
        "expected_conditions": fixed_pressure_expected_conditions(),
...
__all__ = [
    ...
    "case_id_for",
    "fixed_pressure_expected_conditions",
