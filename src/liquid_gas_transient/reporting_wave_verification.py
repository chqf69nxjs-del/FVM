...
def _artifact_manifest(artifact_root: Path | None, metrics_path: Path, summary_path: Path, output_path: Path, metrics: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if artifact_root:
        ...
        for pat in ("*sweep_report.md", "*mesh_overlay*.png", "*cfl_overlay*.png", "*speed_error*.png", "*amplitude_ratio*.png", "*fwhm_broadening*.png", "*waveform_difference*.png"):
...
def _figure_lines(artifact_root: Path | None, output_path: Path, expected: list[tuple[str, str]]) -> list[str]:
    ...
    for label, pattern in expected:
        found = sorted(artifact_root.glob(pattern))
        if not found:
            lines.append(f"- {label}: not found / not included（壊れたMarkdown linkは生成しません）")
            continue
        for p in found:
            rel = p.relative_to(output_path.parent) if p.resolve().is_relative_to(output_path.parent.resolve()) else p
...
def generate_coolprop_small_amplitude_wave_verification_report(
    ...
    design_status = metrics.get("property_backend_design_status", "not_approved_for_design_use")
    ...
    lines: list[str] = [
        ...
        "**Traceability**", f"- eos_model: {_fmt(metrics.get('eos_model'))}", f"- property_backend_name: {_fmt(metrics.get('property_backend_name', 'coolprop_co2'))}", f"- property_backend_design_status: {_fmt(design_status)}", f"- CoolProp version: {_fmt(prov['coolprop_version'])}", f"- output version: {OUTPUT_VERSION}", "",
        ...
        "## 5. Test matrix", "", "### Mesh comparison", *_md_table(["case_id","n_cells","dx","CFL","step count","runtime","execution pass","remained single phase","budget residual"], [[r.get('case_id'), r.get('n_cells'), r.get('dx_m'), r.get('cfl'), r.get('step_count'), r.get('runtime_seconds'), r.get('overall_observation_run_pass'), r.get('remained_single_phase'), r.get('budget_mass_relative_residual')] for r in mesh_rows]), "", "### CFL comparison", *_md_table(["case_id","n_cells","dx","CFL","step count","runtime","execution pass","remained single phase","budget residual"], [[r.get('case_id'), r.get('n_cells'), r.get('dx_m'), r.get('cfl'), r.get('step_count'), r.get('runtime_seconds'), r.get('overall_observation_run_pass'), r.get('remained_single_phase'), r.get('budget_mass_relative_residual')] for r in cfl_rows]), "",
