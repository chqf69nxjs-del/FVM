...
import liquid_gas_transient.cases.v013_fixed_pressure_reflection as module
...
def test_v013c_default_configuration_and_run_plan_are_stable() -> None:
    ...
    assert cfg.boundary_path_travel_m == 35.0
    ...
    assert all(row["verification_item"] == "V-013C" for row in plan)
    assert all(row["right_boundary"] == "fixed_pressure" for row in plan)
...
def test_v013c_configuration_rejects_invalid_or_out_of_scope_values(kwargs) -> None:
    with pytest.raises(ValueError):
        V013FixedPressureReflectionConfig(**kwargs)
...
def test_fixed_pressure_identity_matches_independent_reference_core() -> None:
    expected = fixed_pressure_expected_conditions()
    outgoing = np.asarray([100.0])
    incoming = reflected_incoming_characteristic(outgoing, boundary="fixed_pressure")
    ...
    incident_velocity = outgoing[0] / (900.0 * 500.0)
    assert boundary_reflection_coefficient("fixed_pressure") == expected[
        "characteristic_reflection_coefficient"
    ]
    assert incoming[0] == pytest.approx(-100.0)
    assert pressure[0] == pytest.approx(expected["boundary_pressure_perturbation_pa"])
    assert velocity[0] / incident_velocity == pytest.approx(
        expected["total_boundary_velocity_to_incident_velocity_ratio"]
    )
    assert expected["pressure_reflection_coefficient"] == -1.0
    assert expected["velocity_reflection_coefficient"] == 1.0
    assert expected["zero_mass_flux_expected"] is False
    assert expected["zero_energy_flux_expected"] is False
...
def test_reflection_path_state_is_fixed(distance, phase, center, dominant) -> None:
    ...
    assert state["primary_boundary_guard_overlap_expected"] is (
...
def test_matched_sample_plan_uses_fixed_times_without_shifting() -> None:
    ...
    assert rows[3]["phase"] == "boundary_contact"
...
importlib.import_module(
    "liquid_gas_transient.cases.v013_fixed_pressure_reflection"
)
prohibited = (
...
def test_specification_snapshot_is_json_ready_and_preserves_guardrails() -> None:
    ...
    assert snapshot["expected_conditions"]["pressure_reflection_coefficient"] == -1.0
    assert snapshot["expected_conditions"]["velocity_reflection_coefficient"] == 1.0
    assert len(snapshot["run_plan"]) == 3
    assert len(snapshot["matched_sample_plan"]) == 7
    assert len(snapshot["probe_plan"]) == 3


def test_v013c_scaffold_has_no_production_or_coolprop_imports() -> None:
