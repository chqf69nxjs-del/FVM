...
def _phase_state(
    ...
    *,
    ...
    e_array = (
        np.full(shape, 100.0, dtype=float)
        if e is None
    ...
    raw_phase = np.where(
        ...
        "twophase",
        np.where(
            phase_array == "single_phase_vapor",
            "gas",
            "liquid",
        ),
    )
    return HEMPhaseState(
        ...
        p=np.full(shape, 1.0e6, dtype=float),
        T=np.full(shape, 280.0, dtype=float),
...
def test_mapping_distinguishes_liquid_and_saturated_liquid_at_quality_zero():
    ...
    regions = derive_boundary_regions(state)
...
def test_mapping_uses_configured_endpoint_intervals_without_clipping():
    ...
    regions = derive_boundary_regions(state, config=config)
...
def test_nonfinite_or_out_of_range_equilibrium_quality_fails(quality):
    state = _phase_state([quality], ["liquid_vapor_two_phase"])
...
def test_inconsistent_single_phase_quality_fails(quality, phase_class):
    state = _phase_state([quality], [phase_class])
...
def test_crossing_mapper_rejects_invalid_endpoint_tolerance(endpoint_tolerance):
    state = _phase_state([0.0], ["compressed_or_subcooled_liquid"])
    config = HEMPhaseClassificationConfig(
        endpoint_tolerance=endpoint_tolerance
    )

    with pytest.raises(HEMLiquidToTwoPhaseCrossingError):
        derive_boundary_regions(state, config=config)
...
def test_transition_summary_counts_each_event_class():
    ...
    summary = classify_transition_events(previous, raw).summary()
...
class _AnalyticDirectPhaseEvaluator:
    def __init__(self):
        self.calls: list[
            tuple[
                np.ndarray,
                np.ndarray,
                HEMPhaseClassificationConfig | None,
            ]
        ] = []
    ...
    def __call__(self, rho, e, *, config=None):
        ...
        self.calls.append(
            (
                np.array(rho_array, copy=True),
                np.array(e_array, copy=True),
                config,
            )
        )
...
def test_raw_detection_uses_rho_e_and_is_independent_of_transported_quality():
    ...
    previous = make_conserved([10.0, 10.0], 0.0, [100.0, 100.0], [0.0, 0.0])
    ...
    assert np.array_equal(
        zero_result.raw.region,
        different_result.raw.region,
    )
    assert np.array_equal(
        zero_result.transitions.event,
        different_result.transitions.event,
    )
...
def test_instantiated_phase_config_is_forwarded_to_direct_evaluator():
    ...
    evaluator = _AnalyticDirectPhaseEvaluator()
    U = make_conserved(10.0, 0.0, 100.0, 0.0)
...
def test_direct_detection_retains_current_nonnegative_internal_energy_guard():
    U = make_conserved(10.0, 0.0, -1.0, 0.0)
...
def test_previous_and_raw_conservative_cell_shapes_must_match():
    ...
    raw = make_conserved([10.0, 10.0], 0.0, [100.0, 110.0], [0.0, 0.0])

    with pytest.raises(
        HEMLiquidToTwoPhaseCrossingError,
        match="matching cell shapes",
    ):
        detect_raw_transition_events(
            previous,
            raw,
...
def test_installed_coolprop_endpoints_map_from_canonical_rho_e():
    ...
    result = evaluate_boundary_regions_from_conserved(U)
