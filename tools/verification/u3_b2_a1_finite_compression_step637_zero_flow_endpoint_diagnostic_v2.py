    ...
    *,
    ...
    if lower_chi == upper_chi:
        return lower, [], {
            ...
            "final_width_chi": 0.0,
            "selected_scalar_value": lower_value,
        }
...
def _run(
    *,
    ...
    ultrafine_rows: list[dict[str, Any]] = []
    for index, chi in enumerate(
        np.linspace(
            ultrafine_lower,
            ultrafine_upper,
    ...
    p0_pipe_mass_rate: float | None = None
    p0_velocity: float | None = None
    p0_mach: float | None = None
    p0_phase: str | None = None
    p0_margin: float | None = None
    if p0_endpoint is not None:
        p0_velocity = float(p0_endpoint["velocity_m_s"])
        p0_mach = float(p0_endpoint["mach"])
        p0_phase = str(p0_endpoint["phase"])
        p0_margin = float(p0_endpoint["stagnation_pressure_pa"] - back_pressure)
        p0_pipe_mass_rate = float(
            p0_endpoint["density_kg_m3"] * p0_velocity * hook.area_m2
        )
        velocity_condition = abs(p0_velocity) <= velocity_zero_tolerance
        if velocity_endpoint is not None:
            ...
            velocity_condition = bool(
                velocity_condition or endpoint_chi_separation <= broad_spacing
            )
        zero_endpoint_gate = bool(
            not ultrafine_blocks
            and math.isfinite(p0_pipe_mass_rate)
            and abs(p0_pipe_mass_rate) <= ROOT_TOLERANCE
            and p0_velocity >= -velocity_zero_tolerance
            and 0.0 <= p0_mach < 1.0
            and normalize_phase(p0_phase) in allowed_phases
    ...
    summary = {
        ...
        "stagnation_pressure_endpoint_margin_pa": p0_margin,
        "stagnation_pressure_endpoint_static_pressure_pa": (
            None if p0_endpoint is None else float(p0_endpoint["pressure_pa"])
        ),
        "stagnation_pressure_endpoint_velocity_m_s": p0_velocity,
        "stagnation_pressure_endpoint_mach": p0_mach,
        "stagnation_pressure_endpoint_phase": p0_phase,
        "stagnation_pressure_endpoint_pipe_mass_rate_kg_s": p0_pipe_mass_rate,
...
def main() -> None:
    ...
    _, U, _, parent_root = _verify_parent(
    ...
    _write_csv(output / "step637_root_topology.csv", evidence["root_topology_rows"])
